...
func TestListSpaceQuotas(t *testing.T) {
	Convey("List Space Quotas", t, func() {
		mocks := []MockRoute{
			{"GET", "/v2/space_quota_definitions", listSpaceQuotasPayloadPage1, ""},
			{"GET", "/v2/space_quota_definitions_page_2", listSpaceQuotasPayloadPage2, ""},
		}
...
func TestGetSpaceQuotaByName(t *testing.T) {
	Convey("Get Space Quota By Name", t, func() {
		setup(MockRoute{"GET", "/v2/space_quota_definitions", listSpaceQuotasPayloadPage2, ""}, t)
		defer teardown()
		c := &Config{
			ApiAddress: server.URL,
			Token:      "foobar",
