...
func TestCreateSecurityGroup(t *testing.T) {
	Convey("Create  Security Group With Minimal Parameters", t, func() {
		...
		securityGroups, err := client.CreateSecurityGroup(v3.CreateSecurityGroupRequest{
			Name: "my-sec-group",
		})
		So(err, ShouldBeNil)
	...
	Convey("Create  Security Group With Optional Parameters", t, func() {
		...
		icmpType := 8
		icmpCode := 0
		createSecGroupRequest := v3.CreateSecurityGroupRequest{
			Name: "my-sec-group",
			GloballyEnabled: &v3.GloballyEnabled{
				Running: true,
				Staging: false,
			},
			Rules: []*v3.Rule{
			...
			Relationships: map[string]v3.ToManyRelationships{
				"running_spaces": {
	...
	Convey("Create  Security Group with empty icmp type and code and no name", t, func() {
		...
		_, err = client.CreateSecurityGroup(v3.CreateSecurityGroupRequest{
			Rules: []*v3.Rule{
				{
					Protocol:    "icmp",
					Destination: "10.10.11.0/24",
...
func TestUpdateSecurityGroup(t *testing.T) {
	Convey("Update  Security Group with empty type and code", t, func() {
		...
		_, err = client.UpdateSecurityGroup("guid-1", v3.UpdateSecurityGroupRequest{
			Rules: []*v3.Rule{
				{
					Protocol:    "icmp",
					Destination: "10.10.11.0/24",
	...
	Convey("Update name of  Security Group", t, func() {
		...
		securityGroups, err := client.UpdateSecurityGroup("guid-1", v3.UpdateSecurityGroupRequest{
			Name: "my-sec-group",
		})
		So(err, ShouldBeNil)
		So(securityGroups, ShouldNotBeNil)
	...
	Convey("Update  Security Group With Optional Parameters", t, func() {
		...
		icmpType := 8
		icmpCode := 0
		updateSecGroupRequest := v3.UpdateSecurityGroupRequest{
			Name: "my-sec-group",
			GloballyEnabled: &v3.GloballyEnabled{
				Running: true,
				Staging: false,
